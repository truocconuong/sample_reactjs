...
function getColor({ group, key, mode, }) {
    const rule = group[key] || group.default;
    return rule[mode];
}
function getColors({ appearance, key, mode, }) {
    return {
        background: getColor({
...
export function getCss({ appearance, spacing, mode, isSelected, shouldFitContainer, isOnlySingleIcon, }) {
...
// inline-flex child
export function getIconStyle({ spacing }) {
    return {
        alignSelf: 'center',
        display: 'flex',
...
// inline-flex child
export function getContentStyle({ spacing }) {
    return {
...
export function getFadingCss({ hasOverlay, }) {
    return {
