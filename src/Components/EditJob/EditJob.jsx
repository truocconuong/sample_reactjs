...
export default class PostJob extends Component {
	...
	async getInitData() {
		try {
			...
			// console.log(id_job, 'lala');
			const response = await api.get(`/jobs/${id_job}`);
			if (response) {
				if (this._isMounted) {

					await this.setState({
						tags: [...response.required],
						tagsTwo: [...response.notRequired],
						title: response.title,
						location: response.location.id,
						salary: response.salary,
						time: response.time,
						type: response.type,
						aboutFetch: response.aboutFetch,
						aboutClient: response.aboutClient,
						responsibilities: response.responsibilities,
						enable: response.enable,
						why: response.why,
						required: response.requirement,
						niceToHave: response.niceToHave,
						loading: false
					});
					this.editorOne.current.setContent(this.state.aboutFetch);
					this.editorTwo.current.setContent(this.state.aboutClient);
					this.editorThree.current.setContent(this.state.responsibilities);
					this.editorFour.current.setContent(this.state.required);
					this.editorFive.current.setContent(this.state.niceToHave);
					this.editorSix.current.setContent(this.state.why);
					window.scroll({
						top: 0,
						left: 0,
						behavior: 'smooth'
	...
	async handleSubmit(event) {
		...
		switch (this.state.location) {
			case '1': {
				content = `<p>FETCH - FETCH Cầu Giấy, 3rd Floor, Dai Phat Building - Lane 82, Duy Tan Street, Dich Vong Hau ward, Cau Giay District, Ha Noi City</p><p><span style="color: rgb(136, 136, 136);">${this
					.state.type}</span></p>`;
				break;
			}
			case '2': {
				content = `<p>FETCH - FETCH Hồ Bá Phấn, 42B Ho Ba Phan Street, Phuoc Long A Ward, District 9, Ho Chi Minh City</p><p><span style="color: rgb(136, 136, 136);">${this
					.state.type}</span></p>`;
				break;
			}
			case '3': {
				content = `<p>FETCH - FETCH Điện Biên Phủ, 3rd Floor, 194 Golden Building - 473 Dien Bien Phu Street, Ward 25, Binh Thanh District, Ho Chi Minh City</p><p><span style="color: rgb(136, 136, 136);">${this
					.state.type}</span></p>`;
				break;
			}
			case '4': {
				content = `<p>FETCH - Up Co-working Space - Hai Ba Trung District, 7th Floor, Hanoi Creative City Building - 1 Luong Yen Street, Hai Ba Trung District, Ha Noi City</p><p><span style="color: rgb(136, 136, 136);">${this
					.state.type}</span></p>`;
				break;
			}
			case '5': {
				content = `<p>FETCH  - FETCH HCM/HN</p><p>- Ho Chi Minh: 3rd Floor, 194 Golden Building - 473 Dien Bien Phu Street, Ward 25, Binh Thanh District.</p><p>- Ha Noi: 3rd Floor, Dai Phat Building - Lane 82, Duy Tan Street, Dich Vong Hau Ward, Cau Giay District</p><p><span style="color: rgb(136, 136, 136);">${this
					.state.type}</span></p>`;
				break;
			}
			case '6': {
				content = `<p>FETCH - 364 Cong Hoa Street, Tan Binh District, Ho Chi Minh City</p><p><span style="color: rgb(136, 136, 136);">${this
					.state.type}</span></p>`;
				break;
			}
			case '7': {
				content = `<p>FETCH - Onsite, Singapore, <span style="color: rgb(136, 136, 136);">${this.state
					.type}</span></p>`;
			}
		}
		let dataDesc = '';
		const valueOne = this.editorOne.current.getContent();
		const valueTwo = this.editorTwo.current.getContent();
		...
		try {
			...
			this.setState({ validated: false, loading: true });
			console.log(data);
			const response = await api.put(`/jobs/${this.props.match.params.id}`, data);
			if (response) {
				this.setState({
					loading: false
				});
	...
	setTagSkill = async () => {
		...
		for (let i = 0; i < allSkill.length; i++) {
			if (cata.toLowerCase().indexOf(allSkill[i].name.toLowerCase()) !== -1) {
				if (temp_tag.indexOf(allSkill[i]) == -1) {
	...
	setTagSkillNotRequired = async () => {
		...
		var temp_tag_two = self.state.tagsTwo;
		var allSkill = this.state.listSkill;
		for (let i = 0; i < allSkill.length; i++) {
			if (cata.toLowerCase().indexOf(allSkill[i].name.toLowerCase()) !== -1) {
				if (temp_tag_two.indexOf(allSkill[i]) == -1) {
		...
		if (this.state.tags.length > 0) {
			var filteredArray = temp_tag_two.filter(function (array_el) {
				return (
					self.state.tags.filter(function (anotherOne_el) {
						return anotherOne_el.id == array_el.id;
					}).length == 0
				);
			});
	...
	render() {
		return (
			<div className="main-content" id="panel" style={{ background: 'white' }}>
				<PageLoader display={this.state.loading} />
				<ToastContainer />
				<h3 style={{ paddingTop: '30px', textAlign: 'center' }}>Edit Job To Fetch Tech</h3>
						...
						<div className="row" style={{ paddingTop: '20px' }}>
							...
							<div className="col-md-10">
								...
								>
									...
									<option value="4">Onsite - Singapore</option>
									<option value="5">Fetch HCM/HN</option>
									<option value="6">
              ...
              <div className="col-md-10">
                ...
                />
						...
						<div className="row" style={{ paddingTop: '30px', fontSize: '20px', paddingLeft: '15px' }}>
							Description:
						</div>
						<div className="row" style={{ paddingTop: '30px' }}>
							<div className="col-md-2">
						...
						<div className="row" style={{ paddingTop: '30px' }}>
							<div className="col-md-2">
								<Form.Label style={{ fontSize: '16px' }}>RESPONSIBILITIES:</Form.Label>
							</div>
							<div className="col-md-10">
						...
						<div className="row" style={{ paddingTop: '30px' }}>
							<div className="col-md-2">
								<Form.Label style={{ fontSize: '16px' }}>REQUIREMENT:</Form.Label>
							</div>
							<div className="col-md-10">
						...
						<div className="row" style={{ paddingTop: '30px' }}>
							<div className="col-md-2">
								<Form.Label style={{ fontSize: '16px' }}>NICE TO HAVE:</Form.Label>
							</div>
							<div className="col-md-10">
								<EditorCustomTwo
									key={5}
									ref={this.editorFive}
									handleTagSkill={this.setTagSkillNotRequired}
								/>
							</div>
						</div>
						...
						<div className="row" style={{ paddingTop: '30px' }}>
							<div className="col-md-2">
								<Form.Label style={{ fontSize: '16px' }}>WHY YOU‘LL LOVE WORKING HERE:</Form.Label>
							</div>
							<div className="col-md-10">
						...
						<div className="row" style={{ paddingTop: '30px' }}>
							<div className="col-md-2">
								<Form.Label style={{ fontSize: '20px' }}>Skill required:</Form.Label>
							</div>
							<div className="col-md-10 form-group form-group-nam">
								...
								<div className="user-invite-container">
									<div className="tags-input-list">
										...
										<div
											className={
												this.state.suggestion == 0 ? 'list-suggess hide' : 'list-suggess'
											}
										>
											<ul>
													...
													function (suggess, index) {
														return (
															<li
																...
																key={index}
																onClick={this.selectTag.bind(this, suggess, index)}
															>
						...
						<div className="row" style={{ paddingTop: '30px' }}>
							<div className="col-md-2">
								<Form.Label style={{ fontSize: '20px' }}>Skill:</Form.Label>
							</div>
							<div className="col-md-10 form-group form-group-nam">
								<div className="kynang">
								...
								<div className="user-invite-container">
									<div className="tags-input-list">
										...
										<div
											className={
												this.state.suggestionTwo == 0 ? 'list-suggess hide' : 'list-suggess'
											}
										>
											<ul>
													...
													function (suggess, index) {
														return (
															<li
																className="suggess-item"
																key={index}
																onClick={this.selectTagTwo.bind(this, suggess, index)}
															>
																<div className="fullname">{suggess.name}</div>
															</li>
														);
