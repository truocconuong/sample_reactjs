...
class Day extends Component {
  constructor(props) {
    ...
    this.state = {
      moment: props.moment,
      listBlockDate: props.listBlockDate // new 
    };
  }
  ...
  _renderDay = (week, day) => {
    ...
    //new code by hoain
    let blockDate = false;
    let listBlockDate = this.state.listBlockDate;
    if(listBlockDate){
      listBlockDate.forEach(date=>{
        if(currentDay.isAfter(moment(date.timeStart).subtract(1,"days"), 'day') && currentDay.isBefore(moment(date.timeEnd).add(1, "days"), 'day')){
          blockDate = true;
        }
      })
    }
    ...
    const isDisabled = disabledMax || disabledMin || disabled || limited || blockDate; //new code added
